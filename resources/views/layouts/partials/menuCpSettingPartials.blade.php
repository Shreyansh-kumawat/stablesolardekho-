...
<ul id="submenu-managecp" class="ml-8 mt-1 space-y-1 {{ request()->routeIs('addNewCp', 'cpList','edit_cp') ? '' : 'hidden' }}">
    ...
    <li>
        <a href="{{ route('addNewCp') }}"
            class="flex items-center gap-2 px-3 py-1.5 rounded transition-colors {{ request()->routeIs('addNewCp') ? 'bg-slate-700 text-white' : 'hover:bg-slate-800' }}">
    ...
    <li>
        <a href="{{ route('cpList') }}"
            class="flex items-center gap-2 px-3 py-1.5 rounded transition-colors {{ request()->routeIs('cpList','edit_cp') ? 'bg-slate-700 text-white' : 'hover:bg-slate-800' }}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
