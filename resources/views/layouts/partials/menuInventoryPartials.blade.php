<button type="button" data-toggle="submenu-manageinventory"
    class="w-full flex items-center justify-between px-3 py-2 rounded-md transition-colors hover:bg-slate-800 {{ request()->routeIs('transferInventory','invTxnsAdmin') ? 'bg-slate-700 text-white' : '' }}"
    ...
    <svg class="w-4 h-4 transition-transform {{ request()->routeIs('transferInventory','invTxnsAdmin') ? 'rotate-180' : '' }}"
...
<ul id="submenu-manageinventory"
    class="ml-8 mt-1 space-y-1 {{ request()->routeIs('transferInventory','invTxnsAdmin') ? '' : 'hidden' }}">
