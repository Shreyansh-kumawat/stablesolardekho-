...
@section('css')
    ...
    <style>
        ...
        .unit-rate-input.is-invalid,
        .gst-rate-input.is-invalid {
                ...
                <div class="order-card">
                    <div class="order-detail">
                        ...
                        <div class="order-detail-item">
                            ...
                            <div class="badge-status badge-{{ strtolower($order->status) }}">{{ ucfirst($order->status) }}
                            </div>
                        </div>
                        @if ($order->status=='completed')
                        <div class="order-detail-item">
                            <div class="order-detail-label">Quote Date</div>
                            <div class="order-detail-value">{{ \Carbon\Carbon::parse($order->quote_date)->format('d M, Y') }}</div>
                        </div>
                         <div class="order-detail-item">
                            <div class="order-detail-label">Quote Validity</div>
                            <div class="order-detail-value">{{ \Carbon\Carbon::parse($order->quote_validity_date)->format('d M, Y') }}</div>
                        </div>
                        <div class="order-detail-item">
                            <div class="order-detail-label">Total Amount</div>
                            <div class="order-detail-value">{{ number_format($order->quote_amount, 2) }}</div>
                        </div>
                         <div class="order-detail-item">
                            <div class="order-detail-label">Admin Remarks</div>
                            <div class="order-detail-value">{{ $order->admin_remark ?? 'N/A' }}</div>
                        </div>
                            ...
                            <tbody id="productsTableBody">
                                ...
                                @forelse($products as $product)
                                    ...
                                    <tr class="product-row" data-index="{{ $productIndex }}">
                                        ...
                                        <td>
                                            <strong class="product-gst_rate">{{ $product['gst_rate'] ?? 0 }}</strong>
                                        </td>
                                        <td>
                                            <strong class="product-gst_amount">{{ $product['gst_amount'] ?? 0 }}</strong>
                                        </td>
                                        <td style="line-total">
                                            <strong
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('input', '.unit-rate-input', function() {
                ...
                $(`.line-total[data-index="${index}"]`).text('₹ ' + parseFloat(lineTotal).toLocaleString(
                    'en-IN', {
            ...
            function calculateTotals() {
                ...
                $('#subtotal').text('₹ ' + parseFloat(subtotal).toLocaleString('en-IN', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }));
                $('#gstAmount').text('₹ ' + parseFloat(totalGst).toLocaleString('en-IN', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }));
                $('#grandTotal').text('₹ ' + parseFloat(grandTotal).toLocaleString('en-IN', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }));
            ...
            $('#orderForm').on('submit', function(e) {
                ...
                const details = {
                    ...
                    'GST Amount': '₹ ' + parseFloat(gstAmount).toLocaleString('en-IN', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    }),
                    'Grand Total': '₹ ' + parseFloat(grandTotal).toLocaleString('en-IN', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    })
                };
