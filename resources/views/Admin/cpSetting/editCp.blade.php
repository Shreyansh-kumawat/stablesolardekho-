...
@section('content')
    <div class="page-header">
        <div class="container-fluid">
            <h1><i class="fas fa-user-plus me-2"></i>Edit Channel Partner</h1>
            <p>Edit the details of the channel Partner with role assignment</p>
        </div>
    </div>

    <div class="card">
        <form method="POST" action="{{ route('editCpQuery') }}">
            @csrf
            ...
            <div class="row g-3">
                <div style="display: flex; flex-wrap: wrap; gap: 1rem;">
                    <div style="flex: 1; min-width: 280px;">
                        ...
                        <input id="cp_name" name="cp_name" type="text" required
                            value="{{ old('cp_name', $cp->cp_name ?? '') }}" class="form-control" placeholder="John Doe" />
                    ...
                    <div style="flex: 1; min-width: 280px;">
                        ...
                        <input id="contact_person" name="contact_person" type="text" required
                            value="{{ old('contact_person', $cp->contact_person ?? '') }}" class="form-control" placeholder="John Doe" />
                    ...
                    <div style="flex: 1; min-width: 280px;">
                        ...
                        <input id="cp_email" name="cp_email" type="email" required
                            value="{{ old('cp_email', $cp->email ?? '') }}" class="form-control" placeholder="your.email@example.com" />
                    ...
                    <div style="flex: 1; min-width: 280px;">
                        ...
                        <input id="mobile" name="mobile" type="text"
                            value="{{ old('mobile', $cp->phone_number ?? '') }}" class="form-control"
                            placeholder="+91 98765 43210" />
                        @error('mobile') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>

                    <div style="flex: 1; min-width: 280px;">
                        <label for="role_id" class="form-label">Channel Partner Role</label>
                        <select id="role_id" name="role_id" class="select2-element form-control" required>
                            <option value="">Select CP Type</option>
                            @foreach($cp_roles as $cpRole)
                                <option value="{{ $cpRole->id }}"
                                    {{ (int) old('role_id', $cp->cp_role ?? 0) === (int) $cpRole->id ? 'selected' : '' }}>
                                    {{ $cpRole->role_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div style="flex: 1; min-width: 280px;">
                        ...
                        <select id="state" name="state" class="select2-element form-control" required>
                            <option value="">Select State</option>
                            @foreach($states as $state)
                                <option value="{{ $state }}"
                                    {{ trim(old('state', $cp->state ?? '')) === trim($state) ? 'selected' : '' }}>
                                    {{ $state }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div style="flex: 2; min-width: 300px;">
                        ...
                        <input id="full_address" name="full_address" type="text" required
                            value="{{ old('full_address', $cp->full_address ?? '') }}" class="form-control" placeholder="Street address" />
                        @error('full_address') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>

                    <div style="flex: 1; min-width: 200px;">
                        <label for="pin_code" class="form-label">Pin Code</label>
                        <input id="pin_code" name="pin_code" type="text" required
                            value="{{ old('pin_code', $cp->zip_code ?? '') }}" class="form-control" placeholder="123456" pattern="[0-9]{6}" />
                        @error('pin_code') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="mt-4 d-flex gap-2">
                    <button type="submit" class="btn-primary-theme"><i class="fa fa-user-plus me-2"></i>Update Data</button>
