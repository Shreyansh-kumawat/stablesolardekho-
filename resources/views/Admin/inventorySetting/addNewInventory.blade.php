...
@section('content')
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <form id="inventoryForm" action="{{ route('storeNewInventory') }}" method="POST">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="supplier_id" name="supplier_id" class="form-select select2" required>
                                ...
                                @foreach(($suppliers ?? []) as $supplier)
                                    <option value="{{ data_get($supplier, 'id', '') }}">
                                        {{ data_get($supplier, 'cp_name', data_get($supplier, 'cp_name', '')) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            <label class="form-label">Purchase Value (Unit Price)</label>
        ...
        $(function () {
            ...
            $('#inventoryForm').on('submit', function (e) {
                ...
                if (!$('#sub_category_id').val()) {
                    $('#sub_category_id').next('.select2').find('.select2-selection').addClass('is-invalid');
                    valid = false;
                }

                if (!$('#product_id').val()) {
                    $('#product_id').next('.select2').find('.select2-selection').addClass('is-invalid');
                    valid = false;
                }
