...
<div class="page-header">
    <div class="container-fluid">
        <h1><i class="fas fa-money-check-alt me-2"></i>Fund Deduction</h1>
        <p>Deduct funds from a channel partner with purpose and remarks</p>
    </div>
</div>

<div class="card">
    <form id="fundTransferForm" method="POST" action="{{ route('storeFundDeduction') }}">
        @csrf
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select name="cp_id" id="cp_id" class="select2-element form-control" required>
                    ...
                    @foreach($cp_list as $cp)
                        <option value="{{ $cp->id }}" data-balance="{{ $cp->wallet->balance ?? 0 }}">
                            {{ $cp->cp_name }}
                        </option>
                    @endforeach
                </select>
                <small id="wallet_balance_text" class="text-muted d-block mt-1"></small>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="purpose" id="purpose" class="select2-element form-control" required>
                    ...
                    @foreach(($purposes ?? ['Return', 'Settlement', 'Commission Adjustment', 'Adjustment', 'Other']) as $purpose)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateBalance() {
                const balance = $('#cp_id option:selected').data('balance');
                if (balance === undefined || $('#cp_id').val() === '') {
                    $('#wallet_balance_text').text('');
                } else {
                    $('#wallet_balance_text').text(`Current Wallet Balance: ${balance}`);
                }
            }

            $('#cp_id').on('change', updateBalance);
            updateBalance();
